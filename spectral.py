...
class Chebyshev(Basis):
    ...
    def grid(self, scale=1):
        ...
        i = np.arange(N_grid)
        beta = 0.5*(self.interval[-1]+self.interval[0])
        alpha = 0.5*(self.interval[-1]-self.interval[0])
        return alpha*np.cos((2*i+1)/(2*N_grid)*np.pi)+ beta
...
class BoundaryValueProblem(Problem):
    ...
    def solve(self, F):
        ...
        for sp in self.subproblems:
            ...
            F.gather(sp)
            self.X.vector = spla.spsolve(sp.L, F.vector)
            self.X.scatter(sp)
